<template>
  <main class="w-full text-center p-6">
    ...
    <div class="text-left w-full pt-4">
      ...
      <div :id="project.name" class="cursor-pointer mb-4 flex flex-col sm:flex-row border-2 items-center" @click="(event) => update_displayed_project(project.name, event)">
        <div class="p-2 pr-0">
    ...
    <div v-if="displayed_project != null" id="popup" class="text-left rounded-md border-4 fixed bg-white right-4 left-4 height-0 top-0 p-4 overflow-y-scroll transition-all duration-1000" :style="{ top: targetTop+'px' }">
...
<script setup>
 ...
 async function update_displayed_project(elem_id, event) {
